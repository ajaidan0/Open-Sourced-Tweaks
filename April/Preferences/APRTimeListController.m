...
- (NSArray *)specifiers {
	if (!_specifiers) {
		_specifiers = [self loadSpecifiersFromPlistName:@"APRTimeSettings" target:self];
	}
...
-(void)colorpicker {
	...
	NSString *aprilLabelColor = [prefs objectForKey:@"aprilTimeLabelColor"];
	...
	[alert displayWithCompletion:^void (UIColor *pickedColor) {
		...
		[prefs setObject:hexColor forKey:@"aprilTimeLabelColor"];
	}];
}
