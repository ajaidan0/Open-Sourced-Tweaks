...
- (NSArray *)specifiers {
	if (!_specifiers) {
		_specifiers = [self loadSpecifiersFromPlistName:@"APRWeatherSettings" target:self];
	}
...
-(void)colorpicker {
	...
	NSString *aprilLabelColor = [prefs objectForKey:@"aprilWeatherLabelColor"];
	...
	[alert displayWithCompletion:^void (UIColor *pickedColor) {
		...
		[prefs setObject:hexColor forKey:@"aprilWeatherLabelColor"];
	}];
}
